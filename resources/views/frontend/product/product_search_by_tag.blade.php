...
@section('content')
    ...
    @endif
    <!--- save tag id for per page change -->
            ...
            <div class="col-lg-9 col-md-8 col-12 content-section">
                ...
                <section class="pt-md-3 pt-3 sec-deals product-deals pb-md-5 pb-3" id="product_listing">
                    @if (isset($tag_name) && !empty($tag_name))
                        <h2 id="product_heading" class="mb-4">Products in '{{ $tag_name }}'</h2>
                    @else
                    <h2 id="product_heading" class="mb-4">Search Result</h2>
    ...
    <script>
        function changeResultPerPage() {
            ...
            if (history.pushState) {
                ...
                $.ajax({
                    url:"{{route('ajax.get_per_page_prodcuts_for_tag')}}",
                          type:'post',
                    data:{
                                '_token': '<?= csrf_token()?>',
                        'per_page':per_page,
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.categories').on('change', function () {
                ...
                if (selected_categories.length === 0 && selected_farmers.length === 0)
                {
                    ...
                    $.ajax({
                       url:"{{route('ajax.get_search_result_for_tag_page')}}",
                        data: {
                           '_token':'<?= csrf_token();?>',
                           selected_categories:selected_categories,
                            selected_farmers:selected_farmers,
                    ...
                    $.ajax({
                       url:"{{route('ajax.get_search_result_for_tag_page')}}",
                        ...
                        success:function (result) {
                           $('#product_listing').empty();
                           $('#product_listing').append(result);
                           $('#product_heading').text('Search Results');
        ...
        function  getLatestProducts() {
            ...
            var tag_id =$('#tag_id').val();
                ...
                $.ajax({
                    url:"{{route('ajax.get_latest_products_for_tag_page')}}",
                    data: {
                        'per_page':per_page,
                        'tag_id':tag_id,
                    },
