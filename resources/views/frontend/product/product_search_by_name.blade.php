...
@section('content')
    ...
    @foreach($categories_id as $id)
        ...
        @endforeach
    @endisset
<input type="hidden" name="" value="{{$per_page}}">
<input type="hidden" id="search_product_name" value="{{$search_product_name}}">
    ...
    <main role="content" class="container container-custom">
        ...
        <div class="row m-0">
            ...
            <div class="col-lg-9 col-md-8 col-12 content-section">
                ...
                <h2 class="mb-4">Products '{{$search_product_name}}'</h2> 
                    @else
                    <h2 class="mb-4">Products Near You</h2>
...
@section('javascript')
    <script>
        function changeResultPerPage() {
            ...
            var newurl = window.location.protocol + "//" + window.location.host + window.location.pathname + '?product='+search_product_name+'&page='+current_page_no+'&per_page='+per_page;
        ...
        //get selected categories and farmers
                ...
                $.ajax({
                    url:"{{route('ajax.get_per_page_search_products_by_name')}}",
                           type:'post',
                    data:{
                         '_token': '<?= csrf_token()?>',
                        'per_page':per_page,
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.categories').on('change', function () {
                ...
                if (selected_categories.length === 0 && selected_farmers.length === 0)
                {
                    ...
                    $.ajax({
                       url:"{{route('ajax.get_results_for_search_by_name')}}",
                        data: {
                           '_token':'<?= csrf_token();?>',
                           selected_categories:selected_categories,
                            selected_farmers:selected_farmers,
        ...
        function  getLatestProducts() {
            ...
            var search_product_name =$('#search_product_name').val();
                $.ajax({
                    url:"{{route('ajax.get_latest_products_for_search_by_name')}}",
                    data: {
                        'per_page':per_page,
                        'search_product_name':search_product_name,
                    },
