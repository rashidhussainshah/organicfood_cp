            ...
            <div class="col-lg-9 col-md-8 col-12 content-section">
                ...
                <section class="pt-md-3 pt-3 sec-deals product-deals pb-md-5 pb-3" id="product_listing">
                    ...
                    @if (isset($latest_product) && (!empty($latest_product)))
                        <h2  class="mb-4">{{ $latest_product }}</h2>
                    @elseif (isset($search_result) && (!empty($search_result)))
                        <h2  class="mb-4">{{ $search_result }}</h2>
                    @elseif (isset($nearby_products) && (!empty($nearby_products)))
                        <h2  class="mb-4">{{ $nearby_products }}</h2>
                    @elseif (isset($zip_search) && (!empty($zip_search)))
                        <h2  class="mb-4">Search Result of  <u>'{{ $zip_search }}'</u></h2>
                    @else
...
@section('javascript')
    <script>
        function changeResultPerPage() {
            ...
            if (isShopSearch == 'yes')
            {
                ...
                $.ajax({
                    url:"{{route('ajax.get_per_page_prodcuts_for_shop')}}",
                   type:"post",
                    data:{
                        '_token': '<?= csrf_token()?>',
                        'per_page':per_page,
            ...
            else if(isSearchPage == 'yes')
            {
                
                //update query string for search by zip
                if (history.pushState) {
                 var newurl = window.location.protocol + "//" + window.location.host + window.location.pathname + '?zip_code='+zip_code+'&page='+current_page_no+'&per_page='+per_page;
                  window.history.pushState({path:newurl},'',newurl);
                 }   
                $.ajax({
                    url:"{{route('ajax.get_per_page_prodcuts_for_search_by_zip')}}",
                    type:"post",
                    data:{
                        '_token': '<?= csrf_token()?>',
                        'per_page':per_page,
    ...
    <script>
        $(document).ready(function () {
            //adjust query string if search by zip
            var isSearchPage = $('#isSearchPage').val();
            var zip_code = $('#is_zip_code').val();
           ...
           //already checked category, if user search by category
            ...
            $('.categories').on('change', function () {
                ...
                if (selected_categories.length === 0 && selected_farmers.length === 0)
                {
                    ...
                    $.ajax({
                       url:"{{route('ajax.get_search_result')}}",
                        data: {
                           '_token':'<?= csrf_token();?>',
                           selected_categories:selected_categories,
                            selected_farmers:selected_farmers,
